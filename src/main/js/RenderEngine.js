...
RenderEngine = (function() {
  ...
  RenderEngine.prototype.render = function(input) {
    ...
    for (action in this.actionObjects) {
      if (action === "__defineGetter__" || action === "__defineSetter__") {
        continue;
      }
      aObj = this.actionObjects[action];
      if (this.actionIsIdle(aObj)) {
        continue;
      }
    ...
    for (mapObj in this.map.activeMapObjects) {
      if (mapObj === "__defineGetter__" || mapObj === "__defineSetter__") {
        continue;
      }
  ...
  RenderEngine.prototype.paint = function(obj) {
    var asset, d, i, item, pPoint, rect, tar, _i, _j, _len, _len1, _ref, _ref1, _results, _results1;
    d = obj.bitmapData;
    ...
    if (d.player && d.player.notready === void 0) {
    ...
    } else if (d.player && d.player.notready) {
      ...
      for (i = _j = 0, _len1 = _ref1.length; _j < _len1; i = ++_j) {
        ...
        if (asset.data !== void 0) {
          ...
          _results1.push(this._fgctx.drawImage(asset.data, asset.rect.x, asset.rect.y, asset.rect.width, asset.rect.height, obj.point.x, obj.point.y, asset.rect.width, asset.rect.height));
        } else {
  ...
  RenderEngine.prototype.managePlayerState = function(action, input) {
    ...
    if (this.player.state !== action.state && action.isAnimating === false && action.hasAnimated === false && this.player.isBusy === false) {
      if (action.state === void 0) {
        action.state = this.player.state;
      }
  ...
  RenderEngine.prototype.removeAction = function(action) {
    ...
    if (action.sound) {
      return this.soundEngine.removeSound(action.sound);
    }
  };
...
RenderEngine.prototype.__defineSetter__("map", function(val) {
  if (this._map) {
    this.soundEngine.removeSound(this._map.sound);
  }
