...
CollisionEngine = (function() {
  ...
  CollisionEngine.prototype.fullyEvalulateSuperClass = function(obj) {
    ...
    if (obj instanceof Action) {
      return CollisionEngine.prototype.TYPE_OF_ACTION;
    }
    if (obj instanceof Enemy) {
      return Enemy.prototype.name;
    }
    if (obj instanceof Player) {
      return Player.prototype.name;
    }
  };
  ...
  CollisionEngine.prototype.checkVerticalMapCollision = function(target) {
    var destination, horizontalMax, horizontalMin, i, outOfBounds, point, _i, _j, _ref, _ref1, _ref2, _ref3;
    outOfBounds = this.physicsEngine.isTargetOutOfBounds(target, this.map);
    if (outOfBounds || target.applyGravityAndFriction === false) {
      return;
    }
    ...
    if (target.direction === 1) {
      for (i = _i = _ref = horizontalMin + target.thresholdX, _ref1 = (horizontalMax - target.thresholdX) - 1; _ref <= _ref1 ? _i <= _ref1 : _i >= _ref1; i = _ref <= _ref1 ? ++_i : --_i) {
      ...
      for (i = _j = _ref2 = horizontalMin - target.thresholdX, _ref3 = (horizontalMax + target.thresholdX) - 1; _ref2 <= _ref3 ? _j <= _ref3 : _j >= _ref3; i = _ref2 <= _ref3 ? ++_j : --_j) {
        this.checkForCieling(Math.round(i));
        point = this.map.collisionDataPixel(Math.round(i), Math.round(destination));
        if (point !== 0) {
          this.checkForFloor(Math.round(point), Math.round(destination), Math.round(i), target);
  ...
  CollisionEngine.prototype.checkHorizontalMapCollision = function() {
    var destination, i, point, verticalMax, verticalMin, _i, _ref;
    ...
    for (i = _i = verticalMin, _ref = verticalMax - 1; verticalMin <= _ref ? _i <= _ref : _i >= _ref; i = verticalMin <= _ref ? ++_i : --_i) {
