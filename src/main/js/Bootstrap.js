...
Bootstrap = (function() {
  ...
  Bootstrap.prototype.checkForIE = function() {
    var all, div, i, undef, v, _i;
    undef = "not ie";
    v = 8;
    div = document.createElement('div');
    all = div.getElementsByTagName('i');
    for (i = _i = v; v <= 11 ? _i <= 11 : _i >= 11; i = v <= 11 ? ++_i : --_i) {
  ...
  Bootstrap.prototype.checkAccessors = function() {
    if (!Object.__defineGetter__) {
      ...
      Object.prototype.__defineGetter__ = function(prop, func) {
        if (!_ref.getters[this.name]) {
          _ref.getters[this.name] = {};
        }
    ...
    if (!Object.__defineSetter__) {
      return Object.prototype.__defineSetter__ = function(prop, func) {
        if (!_ref.setters[this.name]) {
          _ref.setters[this.name] = {};
        }
          ...
          for (prop in _ref.setters[obj]) {
            if (prop !== "__defineGetter__" && prop !== "__defineSetter__" && prop !== "name") {
              if (_ref.getters[target.name][prop]) {
                _results1.push(createAccessors(target, prop, _ref.getters[target.name][prop], _ref.setters[target.name][prop]));
              } else {
                _results1.push(createAccessors(target, prop, (function() {}), _ref.setters[target.name][prop]));
              }
            } else {
